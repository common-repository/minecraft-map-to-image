...
class MCM2I_McMap {
	...
	public $imagecreate;			//the GD image handle
	...
	function __construct () {
		...
		$this->nbt = new MCM2I_NBT();
		//populate the default data for a Minecraft map item
		$this->nbt->root[0] = array ('name' => '', 'type' => MCM2I_NBT::TAG_COMPOUND, 'value' => array (
			array ('name' => 'data', 'type' => MCM2I_NBT::TAG_COMPOUND, 'value' => array (
				//1:1 zoom
				array ('name' => 'scale',	'type' => MCM2I_NBT::TAG_BYTE, 	'value' => 0),
				//'overworld' (vs 'nether')
				array ('name' => 'dimension',	'type' => MCM2I_NBT::TAG_BYTE, 	'value' => 0),
				//default map size
				array ('name' => 'height',	'type' => MCM2I_NBT::TAG_SHORT,	'value' => 128),
				array ('name' => 'width',	'type' => MCM2I_NBT::TAG_SHORT,	'value' => 128),
				//locate the map somewhere where the player is unlikely to ever step
				array ('name' => 'xCenter',	'type' => MCM2I_NBT::TAG_INT,		'value' => -12500000),
				array ('name' => 'zCenter',	'type' => MCM2I_NBT::TAG_INT,		'value' => -12500000),
				//start with a blank map
				array (
					'name' => 'colors', 'type' => MCM2I_NBT::TAG_BYTE_ARRAY,
	...
	/* EINDBAAS :: getImage: get the image data ready for display
	   -------------------------------------------------------------------------------------------------------------- */
	...
	public function getPosition() {
		foreach ($this->nbt->root[0]['value'][0]['value'] as &$node) if ($node['name'] == 'xCenter') break;
		$xPosition = $node['value'];
		foreach ($this->nbt->root[0]['value'][0]['value'] as &$node) if ($node['name'] == 'yCenter') break;
		$yPosition = $node['value'];
		return array('x' => (int)$xPosition, 'y' => (int)$yPosition);
	}
	...
	public function getScale() {
		foreach ($this->nbt->root[0]['value'][0]['value'] as &$node) if ($node['name'] == 'scale') break;
		return (int)$node['value'];
	}

	public function getDimension() {
		foreach ($this->nbt->root[0]['value'][0]['value'] as &$node) if ($node['name'] == 'dimension') break;
		return (int)$node['value'];
	}

	public function getDimensionName() {
		foreach ($this->nbt->root[0]['value'][0]['value'] as &$node) if ($node['name'] == 'dimension') break;
		switch ((int)$this->getDimension()) {
			case 0:
